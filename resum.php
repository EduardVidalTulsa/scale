...
$dies="select data from  {$prefixrelacio}index_relacio where data >='2020/10/01' and data <= curdate()  group by data";
$dies="select data from  {$prefixrelacio}index_relacio where data >='2021/01/01' and data <= curdate() and ID_tipus_relacio=3 group by data";
$dies="select data from  {$prefixrelacio}index_relacio where data >='2021/01/01' and data <= '2021/03/31' and ID_tipus_relacio=3 group by data";
$dies="select data from  {$prefixrelacio}index_relacio where data >='2021/04/01' and data <= '2021/06/31' and ID_tipus_relacio=3 group by data";
echo $dies;
br();
$qdies=$link->query($dies) or die($link->error());
while($d=$qdies->fetch_assoc()){
		$q="select * from {$prefixrelacio}index_relacio WHERE `data`='{$d['data']}' and ID_tipus_relacio=3";//and (`ID`='1038' or `ID`='1001')";
		$qlist=$link->query($q);
		echo $q;
		br();
		unset($t);
