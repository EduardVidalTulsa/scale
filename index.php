	...
	case "pesa":
			...
			//
				...
				$qva="SELECT ID_Venedor $FROMindex_relacio where `data` = curdate()
					group by ID_Venedor ORDER BY ID_Venedor limit 8";
//  				echo $qva;
//				pre($_SESSION);
...
// 				echo $uuid['uuid'];
				...
				if( $r['relacio_oberta']=='1'){
					...
					a("{$scale_uri}tiquet.php?uuid={$r['uuid']}","&#128424;");
				}
		 		br();
...
// 				pre($_SESSION['lastline']);
		...
		case "tracto"://quan hem rebut algo al magic
			...
			switch ($ID){
				...
				break;
				case ((strlen($ID)==13)&&($ID[0]==2)):
					echo 'codi barres legacy';
					if (legacyscale($ID)==1){
						legacyinsert($ID);
					}
					$link->close();
					...
					die;
					break;
				case ((is_numeric($ID))&&(strlen($ID)>8)&&($ID[0]!=2))://codi de barres, no tots tenen 13 de longitut, per exemple el oli de coco cuinar
					//echo "ean?";
					$ID=mysql1r("SELECT ID from ean where Value='{$ID}'",$link);
					$link->close();
					header("Location: $scale_uri/?m[estat]=tracto&magic=$ID");
					die;
					break;
				case 'tancament':
					echo "tancament";
				...
				case 'reimprimeix':
					...
					die;
				break;
				case strpos($ID,'sing')!==false:
					list($case,$codi)=explode('+',$ID);
					
					//echo 'sing in at work';
					//die;
					$link->close();
					header ("Location: $sing_uri?w=$w&h=$h&id=".urlencode($codi)."&ID_Empresa=$ID_Empresa&singkey=$singkey&return_uri=".urlencode($scale_uri));
					die;
					break;
				case'-10':
						if (!isset($_SESSION['lastline'])){
...
// 						echo $q; die;
				...
				case is_numeric($ID[0]):
					$sql="select exist,value, formatpvp, bulk, pvp,cost from scalelist where ID=$ID ";
 					//echo $sql;br();die;
					$o=$link->query($sql) or die(header("Location: $scale_uri"));
						...
						if($o[3]==1){//granel
							if(is_numeric($units)&&($units!=" ")){//codi*unitats granel
								...
								if($pes==0){
									$link->close();
									$_SESSION['pes0']++;
// 									a("$?m[estat]=tracto&magic={$_REQUEST['magic']}");
// 									pre($_SESSION);
									if($_SESSION['pes0']>=$repesa){
										$_SESSION['pes0']=0;estil_capcal(false);
										div('error',"style =\"position:absolute; top:".
										($h/2)."px;left:25px;font-size:75px;padding:2px;center;\"");
										a($scale_uri,('pes invàlid apreta aqui sobre'));
										tdiv();
										die;
									}
									else {
										...
										echo "esperant pes correcte [{$_SESSION['pes0']}]";
										tdiv();
										sleep($tempsrepesa);
										//$link->close();
										echo "<script>location.replace(\"{$scale_uri}?m[estat]=tracto&magic={$_REQUEST['magic']}\")</script>";
									}
								}
								$units=$abono.$pes;
								$bulk=1;
				...
				case (($ID[0]=='+')&&(is_numeric($ID[1])))://preu directe
					...
					if(!strpos($altres,'*')){
						//cal treballari més preu directe amb pes i iva diferent de 10% +2+4 per exemple 
						$IVA=$altres;
					}*/
					if (is_numeric($units)){
...
// 						$pes=3;
						if($pes==0){
							...
							if($_SESSION['pes0']>=$repesa){
								...
								tdiv();
								sleep($tempsrepesa);
								echo "<script>location.replace(\"{$scale_uri}?m[estat]=extra&id={$o['4']}\")</script>";
			...
			default:
				$link->close();
				header("Location: $scale_uri?default=$ID");
				break;
		...
		estil_capcal(false);
			...
			if($w<$h) echo '<input type="search" name="venedor" size=7 id="magic" placeholder="Codi venedor o Comanda" class="form-control" autofocus autocomplete="off" onChange="this.form.submit()"; style=" display:table-cell; vertical-align:middle; width:524px; height:80px; position:relative;  font-size: 33px;" />';
	...
	case "entralinea":
		...
		//if($ID=='')header("Location: http://{$_SERVER['HTTP_HOST']}/scale");
		...
		echo ' hola';
		//venda al detall amb venedor
			//echo "prefix: ".$prefixrelacio;Si no tinc taules per aquesta empresa les creeo
			...
			$m["ID"]=$ID_relacio;
			if($new)$m["{$prefixrelacio}line"]["ID_tipus_relacio"]=$ID_tipus_relacio;//1 a relacio_per_assignar segons vars.php i a tipus_relacio al mysql
...
// 			pre($uuid);
			$r=detallrelacio($uuid['uuid'],$ID_Empresa,date("Y"),$link);
			//if ($r['ID_client']=='1')
				$placeholder="&#128241; 1&#129082;&#128182; 2&#129082;&#128179;";//simbol mobil 1->simbol bitllet 2->simbol tarja crèdit
			...
			divclass('caixadescripcio',$scaixa);
			if ($r['phone']=='')echo "Subtotal {$r['tipus_relacio']['Value']} ".mysql1r("SELECT Value from {$ID_Empresa}_Venedor where ID='{$r['ID_Venedor']}' ",$link);
			else echo "{$r['tipus_relacio']['Value']} a ".urldecode($r['fiscal_name'])." {$r['phone']}";
			if($w<$h)br();
...
// 			echo file_get_contents("{$scale_uri}tlinea.php?key=$tlineakey&ID={$_REQUEST['ID']}&ID_Empresa=$ID_Empresa&LSTART={$_REQUEST['LSTART']}&LEND={$_REQUEST['LEND']}");
			tdiv();
			break;
		case "tracto_subtotal":
			...
			if($_REQUEST['magic']=='--5'){
				...
				AND `ID_tipus_relacio` = '{$r['ID_tipus_relacio']}' ;";
				if($link->query($q)===FALSE){
							echo $q.mysqli_error($link);
							die;
						}
			...
			if ($a)header("Location: $scale_uri?m[estat]=subtotal&search=/{$_REQUEST['Venedor']}&ups={$_REQUEST['viapagament']}");
			else header("Location: {$scale_uri}{$data}");
			break;
			}
}
//fi swich estat
	else {// echo "login false 2";
        require('tanca.php');
    }
//$link->close();
?>
