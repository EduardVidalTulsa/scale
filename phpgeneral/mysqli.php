...
function date2mysql($data,$zero_fil=false){
	$any=$data["year"];
        $mes=$data["mon"];
        $dia=$data["mday"];
	if ($zero_fil){
		//echo "true";
		if (($data["mon"]==0)or ($data["mday"]==0) or ($data["year"]==0))
			return "$any-$mes-$dia";
		else return date2mysql($data);
	}
	else {
		if(checkdate($data["mon"],$data["mday"],$data["year"]))
			return "$any-$mes-$dia";
		else {
			error("El dia ".$data["mday"]."/".$data["mon"]."/".$data["year"]." no existeix provo un dia menys" );
			$data["mday"]=$data["mday"]-1;
			if ($data["mday"]<0) die("Dia negatiu");
...
function mysql2vector($query,$link){
	$q=mysqli_query($link,$query)or die (errorr($query).mysqli_error($link));
...
function tableform($config,$link,$debug=false){
	...
	//pre();print_r($camps);print_r($config);tpre();
				...
				if(preg_match("/^id_/",$camp)){
					...
					if (preg_match("/^idW_/",$camp)){
						...
						elseif(preg_match("/^hidden_/",$camp)){
							...
							if($camps[1][$k]=="date"){
								if(($config['today']=="yes") and ($camps[8][$k]=="today")){//check into comment field if contains today
									$data=getdate();
