...
// 		*a($link,$text="")
	...
	Formularis:
		...
		*$name,$size=30,$valor="",$max="255"){ crea una entrada de text a un form amb el nom, valor, etc.
		tform() escriu "</form>"
...
";}
function text($name,$size=30,$valor="",$max="255",$focus=false){
echo"<input type=text name=$name value=\"$valor\" size=$size MAXLENGTH=$max ".($focus=='true'?'autofocus':'')." > \n";
}
function text_js($name,$size=30,$valor="",$max="255",$deco=FALSE,$focus=FALSE,$selectall=false,$style=""){
echo"<input type=text name=$name value=\"".($deco==FALSE?$valor:urldecode($valor))."\" size=$size MAXLENGTH=$max  ".($focus=='true'?'autofocus':'')."  ".($selectall=='true'?'onfocus="this.select();"':'')." onChange=\"this.form.submit()\" $sytle /> \n";
}
function number($name,$min=0,$max=255,$step=0.01 ,$focus=false,$valor="",$style=""){
echo"<input type=number name=$name value=\"$valor\" min=$min MAX=$max step=$step ".($focus=='true'?'autofocus':'')." > \n";
}
function number_js($name,$min=0,$max=255,$step=0.01,$focus='no',$valor="",$style=""){
echo"<input type=number id=$name name=$name value=\"$valor\" min=$min MAX=$max step=$step ".($focus=='true'?'autofocus':'')."  onChange=\"this.form.submit()\" $style > \n ";
}
...
function chek_js($name,$checked=0,$valor="yes"){
if ($checked==0)echo "<input type=\"checkbox\" name=\"$name\" value=\"$valor\" onChange=\"this.form.submit()\">";
else echo "<input type=\"checkbox\" name=\"$name\" value=\"$valor\" checked onChange=\"this.form.submit()\">";
}
function a($link,$text=""){
	echo ar($link,$text);
}
function ar($link,$text="",$target='_self'){
	if($text=="")return "<a href=\"$link\" target=\"$target\">$link</a>";
        else return "<a href=\"$link\" target=\"$target\">$text </a>";
}
...
function select1llista_js($objectiu,$values,$predeterminat=1,$autofocus=false,$style=""){
	$a=array_keys($values);
	echo "<select name=\"$objectiu\" onChange=\"this.form.submit()\" ".($autofocus?' autofocus':'')." "."$style>\n";
...
/*function div($target=body,$extra=''){
	echo "<div id=\"$target\" $extra>";
}*/
